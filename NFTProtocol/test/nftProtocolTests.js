...
createNFTs = async () =>{
    ...
    var contents = [{content: {name: "Ratio Card", image: "ipfs://QmcLZTfyPJxvmGQUKqpiMtVa2qTmPS2MvEPm4gyTkm6mrZ"}, files:["./images/RatioCard/Ratio_Card_Base.png"]}, 
                  /*{content: {name: "Ratio Bad Card", image: "ipfs://QmeZKMuW65SniEtQXrX5quck4ndkrMMvNAKo4bJUXThbJJ", audio:"ipfs://QmRGmpo7UGntccewabLQjRiKk7KvRxcrE7h5Qi5KbL3zm9"}, files: ["./images/RatioCard/RatioCardBase.png", "./sounds/game-success.wav"]
                    {content: {name: "Poly Card Ten Common", image: "ipfs://Qmci4x92WCPfTM6ux53gDBzWU2nqcDg8Q9zt5E5SbvfW6G", model: "ipfs://QmPv3bW8iFLJQAUUawCjJD6ciDbjzECKfFDvPEXczkbhvm"}, files: ["./images/PolygonCards/Card/Complete/IPFS/Ten/Common/PolyCard_Ten_Common.gif", "./models/ethereum_3d_logo/scene.gltf"]} */]
    ...
    for(var c of contents){
        ...
        var info = await createdNFT.contract.info();
...
createNFT2 = async(account, content) =>{
    ...
    var contract = await RatioSingleNFT.new(content.name, "RC", 10000, utils.toWei("1", "ether"), {from: accounts[account]});

    var maxSupply = await contract.contract.methods.maxSupply().call();
...
gateHost2 = async(nft, baseURI, subFiles) =>{
    ...
    for(var subfile of subFiles){
        var file = fs.readFileSync(subfile)
